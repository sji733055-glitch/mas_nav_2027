  ...
  const double * const _R)
{
  ...
  returnvalue =
    globalQPB->init(H, g, lb, ub, nWSRout, &maxCpuTimeOut, x0, 0, (guessedBounds != 0) ? &bounds : 0, _R);
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  if ((strcmp(typeString, "i") == 0) || (strcmp(typeString, "I") == 0)) {
    ...
    if (H_idx >= 0)
      setupHessianMatrix(
        prhs[H_idx], nV, &(globalQP->H), &(globalQP->Hir), &(globalQP->Hjc), &(globalQP->Hv));

    /* make a deep-copy of the user-specified constraint matrix (possibly sparse) */
    if ((nC > 0) && (A_idx >= 0))
      setupConstraintMatrix(
        prhs[A_idx], nV, nC, &(globalQP->A), &(globalQP->Air), &(globalQP->Ajc), &(globalQP->Av));
    ...
    if (isSimplyBoundedQp == BT_TRUE) {
      QProblemB_init(
        handle, globalQP->H, g, lb, ub, nWSRin, maxCpuTimeIn, x0, &options, nlhs, plhs, guessedBounds, R);
    } else {
  ...
  if ((strcmp(typeString, "m") == 0) || (strcmp(typeString, "M") == 0)) {
    ...
    if (nV != (uint_t)globalQP->getNV() || nC != (uint_t)globalQP->getNC()) {
      myMexErrMsgTxt("ERROR (qpOASES): QP dimensions must be constant during a sequence! Try creating a "
                     "new QP instance instead.");
      return;
    }
  ...
  if ((strcmp(typeString, "e") == 0) || (strcmp(typeString, "E") == 0)) {
    ...
    if (returnvalue != SUCCESSFUL_RETURN) {
      char msg[MAX_STRING_LENGTH];
      snprintf(
        msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Couldn't solve current EQP (code %d)!", returnvalue);
      myMexErrMsgTxt(msg);
      return;
    }
