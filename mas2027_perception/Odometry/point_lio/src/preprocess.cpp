...
void Preprocess::mid360_handler(
    const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
  {
    ...
    sensor_msgs::PointCloud2ConstIterator<float> intensity(*msg,
    "intensity");
    sensor_msgs::PointCloud2ConstIterator<double> timestamp(*msg,
    "timestamp");
    ...
    for (; x != x.end();
         ++x, ++y, ++z, ++intensity, ++timestamp, ++index) {
      if (index % point_filter_num != 0) {
        continue;
      }
      ...
      // MID360 timestamp 是绝对秒，Point-LIO curvature 使用毫秒
      double relative_time_ms =
        (*timestamp - frame_stamp) * 1000.0;

      // 允许少量浮点误差
      if (relative_time_ms < -0.001) {
        continue;
      }

      relative_time_ms = std::max(0.0, relative_time_ms);

      const double range2 =
        (*x) * (*x) + (*y) * (*y) + (*z) * (*z);

      if (range2 < blind * blind ||
          range2 > det_range * det_range) {
        continue;
      }
      ...
      // Point-LIO 内部时间单位为毫秒
      point.curvature = relative_time_ms;
